    ...
    protected function doDefine(Form $form)
    {
        ...
        $this->group('search', function (Form $form) {
            ...
            $this->list('field')
                ...
                ->option(__('admin.hero.field.title'), 'hero.title')
                ->option(__('admin.hero.field.alias'), 'hero.alias');
         ...
         *
         * You can override filter actions in HerosModel::configureFilters()
         */
        $this->group('filter', function (Form $form) {
            // State
            $this->list('hero.state')
                ->label('State')
                ->addClass('has-select2')
                // Add empty option to support single deselect button
                ->option('', '')
                ->option(__('admin.hero.filter.state.select'), '')
